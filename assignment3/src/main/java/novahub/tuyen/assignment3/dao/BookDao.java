...
public class BookDao {
	...
	public List<Book> getListSearchBookByUserLogin(int idUser, String keySearch) {
		String sql = "SELECT * FROM book WHERE title LIKE '%" + keySearch + "%'\n" + "AND idUser = " + idUser
				+ " UNION " + "SELECT * FROM book WHERE author LIKE '%" + keySearch + "%'\n" + "AND idUser = " + idUser;
		return jdbcTemplate.query(sql, new BookMapper());
	}

	public List<Book> getListSearchAllBook(String keySearch) {
		String sql = "SELECT * FROM book WHERE title LIKE '%" + keySearch + "%'\n" + " UNION "
				+ " SELECT * FROM book INNER JOIN ON user.idUser = book.idUser WHERE firstName LIKE '%" + keySearch
				+ "%' OR lastName LIKE '%" + keySearch + "%'\n" + " UNION " + "SELECT * FROM book WHERE author LIKE '%"
				+ keySearch + "%'\n";
		return jdbcTemplate.query(sql, new BookMapper());
	}
